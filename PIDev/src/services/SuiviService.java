...
//            else if(message.contains(""))
    ...
    public boolean verifexistant(Suivi suiv){
        if(suiv.getClient().isEmpty() || suiv.getDate_ds() == null || suiv.getDate_fs() == null || suiv.getTemps_ds() == null || suiv.getTemps_fs() == null) return true;
        return false;
    }
    ...
    public ArrayList<Suivi> AfficherSuivi(){
        ...
        try {
            ...
            switch(userclient.getType()){
                case "simple":
                  requete = "select * from suivi WHERE client=?";
                break;
                case "psy":
                    requete = "select * from suivi WHERE username=?";
                break;
            }
            PreparedStatement pst = cnx.prepareStatement(requete);
            pst.setString(1,userclient.getUsername());
