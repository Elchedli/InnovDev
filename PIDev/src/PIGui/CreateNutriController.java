 ...
 * @author HP
 */
public class CreateNutriController implements Initializable {
    ...
    private void ajouter_psy(ActionEvent event) throws IOException 
    {
        ...
        else if (tfDate.getValue().getYear() > 2003)
        {
        ...
        else
        {
            ...
            sn.Ajouter(new nutri (tfCode.getText(), tfUsername.getText(), tfPassword.getText(), tfMail.getText(), java.sql.Date.valueOf((tfDate.getValue())) )) ;

            JOptionPane.showMessageDialog(null,"Welcome sir ♥");

            FXMLLoader loader= new FXMLLoader(getClass().getResource("ProfilNutri.fxml"));
            Parent root= loader.load();
            tfUsername.getScene().setRoot(root);

            ProfilNutriController pc= loader.getController();
